...
function createTranslationItem(html) {
  ...
  var fromDescription = $('.FrWrd').next()[0].childNodes[0].nodeValue;
  var toType = $('.ToWrd em').text();
  $('.ToWrd em').remove();
  var to = $('.ToWrd').text();
  var toDescription = $('.FrWrd').next().find('span').text();
  return {
    from: from.replace('⇒', ''),
    fromType: fromType,
    fromDescription: fromDescription.trim().replace('(', '').replace(')', ''),
    toType: toType,
    to: to.replace('⇒', ''),
    toDescription: toDescription.trim().replace('(', '').replace(')', ''),
    example: {
      from: [],
      to: []
    }
